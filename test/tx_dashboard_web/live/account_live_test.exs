defmodule TxDashboardWeb.AccountLiveTest do
  ...
  @create_attrs %{account: "some account", last_name: "some last_name", name: "some name"}
  ...
  describe "Index" do
    ...
    test "saves new account", %{conn: conn} do
      ...
      assert_patch(index_live, Routes.account_path(conn, :new))
      ...
      {:ok, _, html} =
        ...
        |> render_submit()
        |> follow_redirect(conn, Routes.account_path(conn, :index))
    ...
    test "updates account in listing", %{conn: conn, account: account} do
      ...
      assert_patch(index_live, Routes.account_path(conn, :edit, account))
  ...
  describe "Show" do
    ...
    test "updates account within modal", %{conn: conn, account: account} do
      ...
      assert_patch(show_live, Routes.account_path(conn, :edit, account))
      ...
      {:ok, _, html} =
        ...
        |> form("#account-form", account: @update_attrs)
        |> render_submit()
        |> follow_redirect(conn, Routes.account_path(conn, :show, account))
